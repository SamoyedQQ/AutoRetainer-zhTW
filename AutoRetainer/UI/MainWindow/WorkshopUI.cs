...
internal static unsafe class WorkshopUI
{
    ...
    internal static void Draw()
    {
        ...
        foreach(var data in sortedData.Where(x => x.OfflineAirshipData.Count + x.OfflineSubmarineData.Count > 0 && !x.ExcludeWorkshop))
        {
            ...
            ImGui.PopFont();
            ImGuiEx.Tooltip($"在此角色的多角模式中啟用潛水艇");
            ImGui.SameLine(0, 3);
            ...
            if(data.AreAnyInvalidRedeploysActive())
            {
                ...
                ImGui.PopFont();
                ImGuiEx.Tooltip($"有解鎖方案被設為強制執行時，重新派出會保持啟用。");
                ImGui.SameLine(0, 3);
            }
        ...
        if(C.Verbose)
        {
            if(ImGui.CollapsingHeader("公開除錯資訊"))
            {
                try
                {
                    if(!P.TaskManager.IsBusy)
                    {
                        ...
                        if(ImGui.Button("選擇含 1 個解鎖點的最佳路線"))
                        {
                        ...
                        {
                            if(ImGui.Button($"修理 {x.Name} 潛水艇的損壞配件"))
                            {
    ...
    private static void DrawRow(OfflineCharacterData data, OfflineVesselData vessel, VoyageType type)
    {
        ...
        if(ImGuiEx.BeginPopupNextToElement(n))
        {
            ...
            {
                if(ImGui.BeginCombo("##selprev", "選擇先前的載具名稱", ImGuiComboFlags.HeightLarge))
                {
            ...
            if(C.Verbose)
            {
                if(ImGui.Button("假裝就緒")) vessel.ReturnTime = (uint)P.Time;
                if(ImGui.Button("假裝就緒＋")) vessel.ReturnTime += 60u * (ImGui.GetIO().KeyCtrl ? 10u : 1u) * (ImGui.GetIO().KeyShift ? 10u : 1u);
                if(ImGui.Button("假裝就緒－")) vessel.ReturnTime -= 60u * (ImGui.GetIO().KeyCtrl ? 10u : 1u) * (ImGui.GetIO().KeyShift ? 10u : 1u);
                if(ImGui.Button("假裝未就緒")) vessel.ReturnTime = (uint)(P.Time + 9999);
            }
            ImGui.EndPopup();
        }
        ImGui.PopID();
