...
public unsafe class InventoryManagementCommon
{
    ...
    public void DrawListNew(Action<uint> addAction, Action<uint> removeAction, IReadOnlyList<uint> itemList, Action<uint> additionalButtons = null, Predicate<Item> filter = null)
    {
        ...
        ImGui.SameLine();
        if(ImGuiEx.IconButtonWithText(FontAwesomeIcon.Paste, "與剪貼簿合併", ImGuiEx.Ctrl))
        {
            try
            {
        ...
        ImGuiEx.TreeNodeCollapsingHeader("Mass addition/removal", () =>
        {
            ...
            if(SelectedCategories.Count > 0)
            {
                ...
                ImGui.SetNextItemWidth(200f);
                Modified |= ImGui.InputInt("最低裝備等級", ref ItemLevelMin);
                ImGui.SetNextItemWidth(200f);
                Modified |= ImGui.InputInt("最高裝備等級", ref ItemLevelMax);
                ...
                if(ImGuiEx.CollapsingHeader($"已選取 {SelectedItems.Count} 項道具，其中 {SelectedItems.Count(x => itemList.Contains(x.RowId))} 項已在清單中###counter"))
                {
                    ...
                    foreach(var x in SelectedItems)
                    {
                        actions.Add(() =>
                        {
                            ...
                            void Tooltip()
                            {
                                ...
                                {
                                    if(ImGuiEx.HoveredAndClicked("點擊可立即把這一項道具加入清單"))
                                    {
                                        addAction(x.RowId);
                                    }
                                }
                                ...
                                {
                                    if(ImGuiEx.HoveredAndClicked("右鍵可立即把這一項道具加入清單", ImGuiMouseButton.Right))
                                    {
                ...
                if(ImGuiEx.IconButtonWithText(FontAwesomeIcon.PlusSquare, "把這些道具加入清單", ImGuiEx.Ctrl))
                {
                    foreach(var x in SelectedItems)
                    {
                ...
                ImGuiEx.Tooltip("按住 CTRL 再點擊");
                if(ImGuiEx.IconButtonWithText(FontAwesomeIcon.MinusSquare, "把這些道具從清單移除", ImGuiEx.Ctrl))
                {
                    foreach(var x in SelectedItems)
                    {
                        removeAction(x.RowId);
    ...
    {
        if(ImGuiEx.Button("從 Discard Helper 匯入捨棄項目", ImGuiEx.Ctrl))
        {
            try
            {
        ...
        ImGuiEx.HelpMarker("如果你有在用 Discard Helper 外掛，可以用這個按鈕匯入它的項目，並與現有項目合併。按住 CTRL 再點擊。");
    }
