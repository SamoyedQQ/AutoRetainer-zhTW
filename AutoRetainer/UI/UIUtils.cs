...
internal static class UIUtils
{
    ...
    public static void DrawTeleportIcons(ulong cid)
    {
        ...
        if(offlineData.GetAllowFcTeleportForSubs() || offlineData.GetAllowFcTeleportForRetainers())
        {
            ...
            ImGui.PopFont();
            ImGuiEx.Tooltip(error ?? $"公會房屋已登錄在 Lifestream 且路徑已設定。重新派遣航行器時會傳送到公會房屋；若有啟用，重新派遣僱員時也會傳送過去。\n地址：{Svc.Data.GetExcelSheet<Aetheryte>().GetRowOrDefault((uint)data.FC.ResidentialDistrict)?.Territory.Value.PlaceNameRegion.Value.Name} 第 {data.FC.Ward + 1} 區 {data.FC.Plot + 1} 號");
            ImGui.SameLine(0, 3);
        }
        if(offlineData.GetAllowPrivateTeleportForRetainers())
        {
            ...
            ImGuiEx.Text(error == null ? null : ImGuiColors.DalamudGrey3, "\ue1b0");
            ImGui.PopFont();
            ImGuiEx.Tooltip(error ?? $"個人房屋已登錄在 Lifestream 且路徑已設定。重新派遣僱員時會傳送到個人房屋。\n地址：{Svc.Data.GetExcelSheet<Aetheryte>().GetRowOrDefault((uint)data.Private.ResidentialDistrict)?.Territory.Value.PlaceNameRegion.Value.Name} 第 {data.Private.Ward + 1} 區 {data.Private.Plot + 1} 號");
            ImGui.SameLine(0, 3);
        }
    }
