...
public sealed class VentureStatsManager
{
    ...
    internal void DrawVentures()
    {
        ...
        ImGui.SameLine();
        ImGui.Checkbox("把優質與非優質合併顯示", ref C.StatsUnifyHQ);
        ImGui.SameLine();
        ImGuiEx.SetNextItemFullWidth();
        ImGui.InputTextWithHint("##search", "篩選道具……", ref Filter, 100);
        var cindex = 0;
        foreach(var cData in Data)
        {
            ...
            {
                if(ImGui.CollapsingHeader($"{Censor.Character(cData.Key)} | 探險總次數：{CharTotal.GetSafe(cData.Key)}###chara{cData.Key}"))
                {
                    display = true;
                }
            }
            ...
            foreach(var x in cData.Value)
            {
                ...
                if(display && num != 0)
                {
                    ...
                    ImGui.SameLine();
                    if(ImGui.CollapsingHeader($"{Censor.Retainer(x.Key)} | 探險次數：{num}###{cData.Key}ret{x.Key}"))
                    {
